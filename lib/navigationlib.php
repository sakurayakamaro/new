...
class global_navigation extends navigation_node {
    ...
    public function __construct(moodle_page $page) {
        ...
        if (get_home_page() == HOMEPAGE_SITE) {
            ...
            $properties = array(
				...
				'icon' => new pix_icon('i/navigationitem', get_string('myhome')),
				'display' => true,
				'classes' => array('homeDashboard'),
    ...
    protected function generate_sections_and_activities(stdClass $course) {
        ...
        foreach ($sections as $key => $section) {
            ...
            foreach ($modinfo->sections[$section->section] as $cmid) {
				...
				- MKA2015 15092015
				- Purpose : Show only related activities in navigation block
				*/
				$activity->knvisual = $cm->knvisual ? $cm->knvisual : 0;
				$activity->knverbal = $cm->knverbal ? $cm->knverbal : 0;
				$activity->klbeginner = $cm->klbeginner ? $cm->klbeginner : 0;
				$activity->klintermediate = $cm->klintermediate ? $cm->klintermediate : 0;
				$activity->kladvance = $cm->kladvance ? $cm->kladvance : 0;
	...
	- Adaptive Moodle Agent.
	- MKA2015 01102015
	- Purpose : Material Sequencing Agent
	*/
    ...
    protected function load_section_activities(navigation_node $sectionnode, $sectionnumber, array $activities, $course = null) {
        ...
        foreach ($activities as $activity) {
			...
			- Adaptive Moodle Agent.
			- MKA2015 01102015
			- Purpose : Show only related activities in navigation block (2)
			- Duplicate Material Sequencing Agent in line 1967
			*/
			
		if(user_has_role_assignment($USER->id,5)){		
			if($this->materialSequencing($activity)){ // <-
