...
class course_modinfo {
    ...
    public function __construct($course, $userid) {
        ...
        // Loop through each piece of module data, constructing it adaptive
...
class cm_info implements IteratorAggregate {
	...
	*/
    ...
    public function __construct(course_modinfo $modinfo, $notused1, $mod, $notused2) {
		...
		- Purpose : Personalize materials that show to learner
		*/		
        $this->knvisual          = $mod->knvisual;
        $this->knverbal          = $mod->knverbal;
        $this->klbeginner        = $mod->klbeginner;
        $this->klintermediate    = $mod->klintermediate;
        $this->kladvance         = $mod->kladvance;
        ...
        if (!isset($modviews[$this->modname])) {
            $modviews[$this->modname] = !plugin_supports('mod', $this->modname, FEATURE_NO_VIEW_LINK);
        }
