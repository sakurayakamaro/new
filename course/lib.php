           ...
           if (!empty($section->sequence)) {
               ...
               foreach ($sequence as $seq) {
                   ...
                   $mod[$seq]->mod              = $rawmods[$seq]->modname;
					...
					- MKA2015 25092015
					- Purpose : Personalize materials that show to learner
					*/
                   $mod[$seq]->knvisual         = isset($rawmods[$seq]->knvisual) ? $rawmods[$seq]->knvisual:0;
                   $mod[$seq]->knverbal         = isset($rawmods[$seq]->knverbal)? $rawmods[$seq]->knverbal:0;
                   $mod[$seq]->klbeginner       = isset($rawmods[$seq]->klbeginner)? $rawmods[$seq]->klbeginner:0;
                   $mod[$seq]->klintermediate   = isset($rawmods[$seq]->klintermediate)? $rawmods[$seq]->klintermediate:0;                   $mod[$seq]->kladvance        = isset($rawmods[$seq]->kladvance)? $rawmods[$seq]->kladvance:0;
                   ...
                   if ($hasfunction = function_exists($functionname)) {
                       if ($info = $functionname($rawmods[$seq])) {
                           ...
                           if (!empty($info->name)) {
							...
							- Purpose : Personalize materials that show to learner
							*/
                           if (!empty($info->knvisual)) {
                               $mod[$seq]->knvisual = $info->knvisual;
                           }
                           if (!empty($info->knverbal)) {
                               $mod[$seq]->knverbal = $info->knverbal;
                           }
                           if (!empty($info->klbeginner)) {
                               $mod[$seq]->klbeginner = $info->klbeginner;
                           }
                           if (!empty($info->klintermediate)) {
                               $mod[$seq]->klintermediate = $info->klintermediate;
                           }
                           if (!empty($info->kladvance)) {
                               $mod[$seq]->kladvance = $info->kladvance;
                           }
                   ...
                   if (!$hasfunction && $rawmods[$seq]->showdescription) {
                       if ($modvalues = $DB->get_record($rawmods[$seq]->modname,
                               array('id' => $rawmods[$seq]->instance), 'name, intro, introformat, knvisual, knverbal, klbeginner, klintermediate, kladvance')) {
                           ...
                           $mod[$seq]->name = $modvalues->name;
								...
								- Purpose : Personalize materials that show to learner
								*/
                               $mod[$seq]->knvisual = $modvalues->knvisual;
                               $mod[$seq]->knverbal = $modvalues->knverbal;
                               $mod[$seq]->klbeginner = $modvalues->klbeginner;
                               $mod[$seq]->klintermediate = $modvalues->klintermediate;
                               $mod[$seq]->kladvance = $modvalues->kladvance;
                       }
                   }
