...
function useredit_shared_definition(&$mform, $editoroptions, $filemanageroptions, $user) {
	...
	* MKA2015 02072015
	*/
	$mform->addElement('header', 'adaptive_moodle', 'Adaptive');
		
	$context = context_user::instance($USER->id);
	// If user is student
	if (!has_capability('moodle/user:create', $context)) {
		$mform->addElement('text', 'cgpa', get_string('cgpa'),'readonly');
		$mform->setType('cgpa', PARAM_NOTAGS);
		$mform->setDefault('cgpa', $USER->cgpa);
	}
	else{
		$mform->addElement('text', 'cgpa', get_string('cgpa'));
		$mform->setType('cgpa', PARAM_NOTAGS);
		$mform->setDefault('cgpa', $USER->cgpa);
	}
	/*........................*/
