 ...
 */
function url_supports2($feature) {
    switch($feature) {
 ...
 * @return array
 */
function url_get_extra_capabilities2() {
 ...
 */
function url_reset_userdata2($data) {
    return array();
}
...
function url_get_coursemodule_info($coursemodule) {
    ...
    if (!$url = $DB->get_record('url', array('id'=>$coursemodule->instance),
            'id, name, display, displayoptions, externalurl, parameters, intro, introformat, knvisual, knverbal, klbeginner, klintermediate, kladvance')) {
        return NULL;
    }
    ...
    $info->name = $url->name;
		...
		- Purpose : Personalize materials that show to learner
		*/
	$info->knvisual = $url->knvisual;
	$info->knverbal = $url->knverbal;
	$info->klbeginner = $url->klbeginner;
	$info->klintermediate = $url->klintermediate;
	$info->kladvance = $url->kladvance;
