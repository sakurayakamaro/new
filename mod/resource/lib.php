...
function resource_get_coursemodule_info($coursemodule) {
    ...
    if (!$resource = $DB->get_record('resource', array('id'=>$coursemodule->instance),
            'id, name, display, displayoptions, tobemigrated, revision, intro, introformat, knvisual, knverbal, klbeginner, klintermediate, kladvance')) {
        return NULL;
    }
    ...
    $info->customdata = resource_get_optional_details($resource, $coursemodule);
		...
		- Purpose : Personalize materials that show to learner
		*/
	$info->knvisual = $resource->knvisual;
	$info->knverbal = $resource->knverbal;
	$info->klbeginner = $resource->klbeginner;
	$info->klintermediate = $resource->klintermediate;
	$info->kladvance = $resource->kladvance;
