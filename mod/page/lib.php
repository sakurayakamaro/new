...
function page_get_coursemodule_info($coursemodule) {
    ...
    if (!$page = $DB->get_record('page', array('id'=>$coursemodule->instance),
            'id, name, display, displayoptions, intro, introformat, knvisual, knverbal, klbeginner, klintermediate, kladvance')) {
        return NULL;
    }
    ...
    $info->name = $page->name;
		...
		- Purpose : Personalize materials that show to learner
		*/
	$info->knvisual = $page->knvisual;
	$info->knverbal = $page->knverbal;
	$info->klbeginner = $page->klbeginner;
	$info->klintermediate = $page->klintermediate;
	$info->kladvance = $page->kladvance;
