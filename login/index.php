...
require_once('../adaptive/agent.php');
...
if ($frm and isset($frm->username)) {                             // Login WITH cookies
    ...
    /// Currently supported only for ldap-authentication module
		 ...
		 * Redirect
		 */
		$urltogo2 = new moodle_url('/adaptive/ilsq.php', array('status' => 0));
		
		// If user_id == student
		if(user_has_role_assignment($user->id,5)){
			// If not yet answer ILS Questionnaire
			if($user->date_taken_ils=='')
        		$SESSION->wantsurl = $urltogo2;				
			else
        		$SESSION->wantsurl = $urltogo;
				
			// Check knowledge level
			// If not set yet, set the knowledge level
			if($user->knowledge_level==''&&$user->cgpa!=''){
				$knowledgeLevel = knowledgeLevelCGPA($user->cgpa);
				
				// Update knowledge level in db user
				$user = new stdClass();
				$user->id = $USER->id;
			...
			// Reset the knowledge level if actual knowledge level not equal to current knowledge level
			elseif($user->knowledge_level!=knowledgeLevelCGPA($user->cgpa)){
				$knowledgeLevel = knowledgeLevelCGPA($user->cgpa);
